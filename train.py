...
def loadText(filename):
	...
	dataX = []
	dataY = []
	for i in range(0, totalChars - seqLength, stepSize):
...
'''
Another update - Changed the learning rate to 0.0005 instead of 0.001 which was default. 
forgot to put the decay in it first and hence the model kept on over shooting the learning gradient a lot and hence less learning
now at 0.0005 and decay of 0.01, that is just 1 percent with every learning, the model is not actually learning more than previous
'''

'''
Update 4 - after just one set of decaying and low learning rate, it turned out, the learning started falling flat.
After tring various things, I got to realize that the network is not complex enough to get this to train.
Hence, by compromising on generalization(hopefully wont happen with the case of text generation using lstm), i got rid of dropout.
'''